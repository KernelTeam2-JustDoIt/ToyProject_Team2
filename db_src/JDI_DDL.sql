...
CREATE TABLE `ACCOMMODATION` (
	...
	`ACCOMMODATION_STATUS`	VARCHAR(10)	NOT NULL,
...
-- ROLE_PRIVILEGE 테이블은 복합키(PRIVILEGE_ID, ROLE_ID)이므로 AUTO_INCREMENT 적용 불가
...
-- USER_COUPON 테이블은 복합키(CUSTOMER_ID, COUPON_ID)이므로 AUTO_INCREMENT 적용 불가
...
-- ACCOMMODATION_POLICY 테이블은 복합키(POLICY_ID, ACCOMMODATION_ID)이므로 AUTO_INCREMENT 적용 불가
...
ALTER TABLE `CUSTOMER_INQUIRY` AUTO_INCREMENT = 1;

-- ACCOMMODATION_WISHLIST 테이블은 복합키(ACCOMMODATION_ID, CUSTOMER_ID)이므로 AUTO_INCREMENT 적용 불가
...
ALTER TABLE `POINT_DETAIL` AUTO_INCREMENT = 1;
...
-- RESERVE_AGREEMENT_VERIFY 테이블은 복합키(RESERVE_AGREEMENT_ID, RESERVE_ID)이므로 AUTO_INCREMENT 적용 불가

-- ACCOMMODATION_FACILITY 테이블은 복합키(ACCMMODATION_ID, FACILITY_ID)이므로 AUTO_INCREMENT 적용 불가
...
-- SELLER_STATUS_HISTORY 테이블의 PK는 SELLER_STATUS_ID인데, 이는 SELLER_STATUS 테이블의 PK와 동일한 이름이므로
-- 이 테이블의 HISTORY_ID 같은 다른 PK가 있을 것으로 예상되지만, 현재 쿼리상으로는 단일 PK로 명시되어 있습니다.
-- 만약 SELLER_STATUS_ID가 다른 테이블에서 FK로 참조될 목적으로 AUTO_INCREMENT가 필요없다면 적용하지 않습니다.
-- 여기서는 단일 PK이므로 일단 적용합니다.
...
ALTER TABLE `POINT_EVENT`
    MODIFY COLUMN `EVENT_ID` INT NOT NULL AUTO_INCREMENT;
ALTER TABLE `POINT_EVENT` AUTO_INCREMENT = 1;
...
ALTER TABLE `CUSTOMER_BENEFIT` AUTO_INCREMENT = 1;
